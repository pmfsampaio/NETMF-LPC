...
#include <AD7991_Driver.h>

#define abs(a) (((a) < 0) ? -(a) : (a))

BOOL AD7991_Driver::Enable( GPIO_INTERRUPT_SERVICE_ROUTINE touchIsrProc )
{
    if (!::CPU_GPIO_EnableInputPin2(
            LPC22XX_GPIO::c_P1_22,
            FALSE,
            touchIsrProc,
            NULL,
            GPIO_INT_EDGE_LOW,
...
BOOL AD7991_Driver::Disable()
{
    return true;
}

void AD7991_Driver::GetPoint( TOUCH_PANEL_SAMPLE_FLAGS* pTipState, int* pSource, int* pUnCalX, int* pUnCalY )
{
    *pTipState = 0;
    *pUnCalX = 0;
    *pUnCalY = 0;
...
BOOL AD7991_Driver::CalibrationPointGet(TOUCH_PANEL_CALIBRATION_POINT *pTCP)
{
...
HRESULT AD7991_Driver::GetDeviceCaps(unsigned int iIndex, void* lpOutput)
{
    if ( lpOutput == NULL )
    {
        return FALSE;
