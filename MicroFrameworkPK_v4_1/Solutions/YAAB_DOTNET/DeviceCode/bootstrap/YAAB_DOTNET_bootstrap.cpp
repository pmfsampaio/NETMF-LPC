...
{
	// PS - BEGIN
		LPC22XX::SYSCON().VPBDIV = 0x00;
		LPC22XX::TIMER(0).TCR = 0x0;
		LPC22XX::TIMER(0).TC = 0x0;
		LPC22XX::TIMER(0).PR = 0x0;
		LPC22XX::TIMER(0).PC = 0x0;
		LPC22XX::TIMER(0).IR = 0x0;
	// PS - END
		// Phytec LPC2294 board uses a 10 MHz crystal
	    // Set PLL to 58.982400 MHz
	    // 14.745600 MHz crystal, P = 4, M = 3, cclk = 58.9824 MHz
	    LPC22XX::SYSCON().PLLCFG  = 0x24; // PS 0x25;
    ...
    LPC22XX::SYSCON().MAMCR = 0;
    LPC22XX::SYSCON().MAMTIM = 4;
    LPC22XX::SYSCON().MAMCR = 2;
}
...
// if a shorter wait state, then rx buffer size can be reduce. say   LPC22XX::EMC().BCFG0=0x20001c63; then rx buffer can be 1024
	...
	LPC22XX::EMC().BCFG0=0x10000C60;//3461;
...
void __section(SectionForBootstrapOperations) BootstrapCode_Pins()
{
	 ...
	 */
	LPC22XX::EMC().BCFG2 = 0x20000c40;

//   111 00 1 1 0 00 01 01 0 0 1 00 1 11 10 01 00
    LPC22XX::PCB().PINSEL2=0x0F814924;// PS 0x0E6149E4;
}
