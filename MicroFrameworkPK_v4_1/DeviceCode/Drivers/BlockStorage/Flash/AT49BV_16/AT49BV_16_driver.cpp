...
//--//
BOOL __section(SectionForFlashOperations) AT49BV_16_BS_Driver::ChipInitialize( void* context )
{
...
BOOL __section(SectionForFlashOperations) AT49BV_16_BS_Driver::ChipUnInitialize( void* context )
{
	return TRUE;
}

const BlockDeviceInfo* __section(SectionForFlashOperations) AT49BV_16_BS_Driver::GetDeviceInfo( void* context )
{
	return ((MEMORY_MAPPED_NOR_BLOCK_CONFIG*)context)->BlockConfig.BlockDeviceInformation;
}

BOOL __section(SectionForFlashOperations) AT49BV_16_BS_Driver::ChipReadOnly(void* context, BOOL On, UINT32 ProtectionKey )
{
	NATIVE_PROFILE_HAL_DRIVERS_FLASH();
...
BOOL __section(SectionForFlashOperations) AT49BV_16_BS_Driver::ReadProductID(void* context, FLASH_WORD& ManufacturerCode, FLASH_WORD& DeviceCode )
{
	NATIVE_PROFILE_HAL_DRIVERS_FLASH();
...
BOOL __section(SectionForFlashOperations) AT49BV_16_BS_Driver::IsBlockErased( void* context, ByteAddress address, UINT32 blockLength )
{
	NATIVE_PROFILE_HAL_DRIVERS_FLASH();
...
BOOL __section(SectionForFlashOperations) AT49BV_16_BS_Driver::EraseBlock( void* context, ByteAddress address )
{
	NATIVE_PROFILE_HAL_DRIVERS_FLASH();
...
BOOL __section(SectionForFlashOperations) AT49BV_16_BS_Driver::Read(void* context, ByteAddress address, UINT32 numBytes, BYTE * pSectorBuff)
{
	NATIVE_PROFILE_HAL_DRIVERS_FLASH();
...
BOOL __section(SectionForFlashOperations) AT49BV_16_BS_Driver::Write(void* context, ByteAddress Address, UINT32 NumBytes, BYTE * pSectorBuff, BOOL ReadModifyWrite)
{
	...
	{
		BOOL r = WriteX(context, Address, NumBytes, pSectorBuff, ReadModifyWrite, TRUE);
		return r;
	}

}

BOOL __section(SectionForFlashOperations) AT49BV_16_BS_Driver::Memset(void* context, ByteAddress address, UINT8 data, UINT32 numBytes)
{
	NATIVE_PROFILE_PAL_FLASH();
	...
	BOOL r=	WriteX(context, address, numBytes, (BYTE*)&chipData, TRUE, FALSE);

	return r;

}

BOOL __section(SectionForFlashOperations) AT49BV_16_BS_Driver::WriteX(void* context, ByteAddress address, UINT32 numBytes, BYTE * pSectorBuff, BOOL ReadModifyWrite, BOOL fIncrementDataPtr)
{
	NATIVE_PROFILE_PAL_FLASH();
...
BOOL __section(SectionForFlashOperations) AT49BV_16_BS_Driver::GetSectorMetadata(void* context, ByteAddress SectorStart, SectorMetadata* pSectorMetadata)
{
	return TRUE;
}

BOOL __section(SectionForFlashOperations) AT49BV_16_BS_Driver::SetSectorMetadata(void* context, ByteAddress SectorStart, SectorMetadata* pSectorMetadata)
{
	return TRUE;
}

void __section(SectionForFlashOperations) AT49BV_16_BS_Driver::SetPowerState(void* context, UINT32 State) {
	// our flash driver is always ON
	return;
}

UINT32 __section(SectionForFlashOperations) AT49BV_16_BS_Driver::MaxSectorWrite_uSec( void* context )
{
	NATIVE_PROFILE_PAL_FLASH();
...
UINT32 __section(SectionForFlashOperations) AT49BV_16_BS_Driver::MaxBlockErase_uSec( void* context )
{
	NATIVE_PROFILE_PAL_FLASH();
...
void __section(SectionForFlashOperations) AT49BV_16_BS_Driver::Action_ReadID(void* context, FLASH_WORD& ManufacturerCode,
		FLASH_WORD& DeviceCode) {
	NATIVE_PROFILE_HAL_DRIVERS_FLASH();

	MEMORY_MAPPED_NOR_BLOCK_CONFIG* config =
			(MEMORY_MAPPED_NOR_BLOCK_CONFIG*) context;

	volatile CHIP_WORD
			* BaseAddress =
					(CHIP_WORD*) config->BlockConfig.BlockDeviceInformation->Regions[0].Start;
	...
	FLASH_BEGIN_PROGRAMMING_FAST();
		...
		BaseAddress[0x0555] = 0x00AA;
		BaseAddress[0x0AAA] = 0x0055;
		BaseAddress[0x0555] = 0x00F0;

	FLASH_END_PROGRAMMING_FAST( "AT49BV_BS_16 ReadProductID", BaseAddress );
	//
	//
	///////////////////////////////////
}

void __section(SectionForFlashOperations)
AT49BV_16_BS_Driver::Action_Unprotect(void* context,
		volatile CHIP_WORD* BaseAddress) {
	NATIVE_PROFILE_HAL_DRIVERS_FLASH();
}

BOOL __section(SectionForFlashOperations) AT49BV_16_BS_Driver::Action_EraseSector( void* context, volatile CHIP_WORD* SectorStart )
{
	NATIVE_PROFILE_HAL_DRIVERS_FLASH();
	...
	*SectorStart = 0x0030;
	...
	FLASH_END_PROGRAMMING_FAST( "AT49BV_BS_16 EraseSector", SectorStart );
	//
	//
	///////////////////////////////////
...
void __section(SectionForFlashOperations)
AT49BV_16_BS_Driver::Action_WriteWord(void* context,
		volatile CHIP_WORD* Address, CHIP_WORD Data) {
	NATIVE_PROFILE_HAL_DRIVERS_FLASH();
		...
		*Address = Data;
		...
		// There is no timeout at here, if it fails, leave it for the system watchdog to reset it.
		{
			...
			while ((*Address & SR_WSM_READY) != (Data & SR_WSM_READY)) {
				...
				if (Data_DQ5 & 0x0020) {
					Data_DQ7 = *Address;
					if ((Data_DQ5 & 0x0020) && ((Data_DQ7 & 0x0080) != (Data
							& 0x00080))) {
	...
	FLASH_END_PROGRAMMING_FAST( "AT49BV_BS_16 WriteWord", Address );
	//
	//
	///////////////////////////////////
}
#if defined(ADS_LINKER_BUG__NOT_ALL_UNUSED_VARIABLES_ARE_REMOVED)
#pragma arm section rodata = "g_AT49BV_16_BS_DeviceTable"
#endif

struct IBlockStorageDevice g_AT49BV_16_BS_DeviceTable = {
		&AT49BV_16_BS_Driver::ChipInitialize,
		&AT49BV_16_BS_Driver::ChipUnInitialize,
		&AT49BV_16_BS_Driver::GetDeviceInfo,
		&AT49BV_16_BS_Driver::Read,
		&AT49BV_16_BS_Driver::Write,
		&AT49BV_16_BS_Driver::Memset,
		&AT49BV_16_BS_Driver::GetSectorMetadata,
		&AT49BV_16_BS_Driver::SetSectorMetadata,
		&AT49BV_16_BS_Driver::IsBlockErased,
		&AT49BV_16_BS_Driver::EraseBlock,
		&AT49BV_16_BS_Driver::SetPowerState,
		&AT49BV_16_BS_Driver::MaxSectorWrite_uSec,
		&AT49BV_16_BS_Driver::MaxBlockErase_uSec, };
