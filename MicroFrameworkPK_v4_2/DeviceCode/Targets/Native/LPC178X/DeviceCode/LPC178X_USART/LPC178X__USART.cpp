...
{
    return LPC178X_USART_Driver::Initialize(ComPortNum, BaudRate, Parity, DataBits, StopBits, FlowValue);
}
...
{   
    return LPC178X_USART_Driver::GetPins(ComPortNum, rxPin, txPin, ctsPin, rtsPin );
}
...
{
    return LPC178X_USART_Driver::BaudrateBoundary(ComPortNum, maxBaudrateHz, minBaudrateHz);
}
...
void LPC178X_USART_Driver::UART_IntHandler (void *param)
{
   ...
   while(!((IIR_Value = USARTC.SEL3.IIR.UART_IIR) & 0x01)) {
	   switch (IIR_Value & LPC178X_USART::UART_IIR_IID_mask) {
	   case 0x4:
	   case 0xc:	// rx
		   do {
			   cr = (UINT8 )USARTC.SEL1.RBR.UART_RBR;
			   put(cr);
			   USART_AddCharToRxBuffer(ComNum, cr);
		   } while (USARTC.UART_LSR & LPC178X_USART::UART_LSR_RFDR);
          //Events_Set( SYSTEM_EVENT_FLAG_COM_IN );
          break;
	   case 0x2: // tx
	   	   if (USART_RemoveCharFromTxBuffer( ComNum, c )) {
	   		   if (USART_BytesInBuffer(ComNum, FALSE) == 0) TxBufferEmptyInterruptEnable( ComNum, FALSE );
   ...
   if (IIR_Value & LPC178X_USART::UART_IIR_NIP)
         return; //ASSERT(0);
   ...
   while (true) 
   {
     ...
     if (LSR_Value & LPC178X_USART::UART_LSR_RFDR)
     {
...
BOOL LPC178X_USART_Driver::Initialize( int ComPortNum, int BaudRate, int Parity, int DataBits, int StopBits, int FlowValue )
{
    ...
    divisor = CLKPWR_GetCLK(CLKPWR_CLKTYPE_PER);
    ...
    if(5 <= DataBits && DataBits <= 8)
    {
        SET_BITS(USARTC.UART_LCR,
                 LPC178X_USART::UART_LCR_WLS_shift,
    ...
    switch(Parity)
    {
        ...
        case USART_PARITY_EVEN:
            USARTC.UART_LCR |= (LPC178X_USART::UART_LCR_EPE | LPC178X_USART::UART_LCR_PBE);
            break;
    ...
    USARTC.SEL3.FCR.UART_FCR =  (LPC178X_USART::UART_FCR_RFITL_08 << LPC178X_USART::UART_FCR_RFITL_shift )  |
                                LPC178X_USART::UART_FCR_TFR      |
                                LPC178X_USART::UART_FCR_RFR      |
    ...
    USARTC.SEL2.IER.UART_IER |= LPC178X_USART::UART_IER_RLSIE;
...
//    ASSERT(LPC178X_USART_Driver::IsValidPortNum(ComPortNum));
    ...
    if (USARTC.UART_LSR & LPC178X_USART::UART_LSR_THRE) return TRUE;
    return FALSE;
}
...
//    ASSERT(LPC178X_USART_Driver::IsValidPortNum(ComPortNum));
    ...
    if (USARTC.UART_LSR & LPC178X_USART::UART_LSR_TE) return TRUE;
    return FALSE;
}
...
void LPC178X_USART_Driver::WriteCharToTxBuffer ( int ComPortNum, UINT8 c )
{
    ...
    // SET_BITS(USARTC.SEL1.THR.UART_THR,LPC178X_USART::UART_THR_DATA_shift,LPC178X_USART::UART_THR_DATA_mask,ui32Char);
    
    // special handling to newline ('\n' -> '\n\r').
   // if (c == '\n') WriteCharToTxBuffer(ComPortNum, '\r');
}

void LPC178X_USART_Driver::TxBufferEmptyInterruptEnable( int ComPortNum, BOOL Enable  )
{
    GLOBAL_LOCK(irq);
...
//    ASSERT(LPC178X_USART_Driver::IsValidPortNum(ComPortNum));
    ...
    if (Enable)
    {
//        USARTC.UART_LCR &= (~LPC178X_USART::UART_LCR_DLAB);
...
//    USARTC.UART_LCR &= (~LPC178X_USART::UART_LCR_DLAB);
    ...
    if (USARTC.SEL2.IER.UART_IER & (LPC178X_USART::UART_IER_THREIE)) return TRUE;
    return FALSE;
}

void LPC178X_USART_Driver::RxBufferFullInterruptEnable ( int ComPortNum, BOOL Enable  )
{
    GLOBAL_LOCK(irq);
...
BOOL LPC178X_USART_Driver::RxBufferFullInterruptState ( int ComPortNum )
{
 ...
 //   ASSERT(LPC178X_USART_Driver::IsValidPortNum(ComPortNum));
    ...
    if (USARTC.SEL2.IER.UART_IER & (LPC178X_USART::UART_IER_RDAIE)) return TRUE;
    return FALSE;
...
void LPC178X_USART_Driver::ProtectPins ( int ComPortNum, BOOL On )
{
    ...
    switch(ComPortNum)
    {
    case c_COM1: SER_TXD = LPC178X_USART::c_SER1_TXD; SER_RXD = LPC178X_USART::c_SER1_RXD; PinsProtected = &COM1_PinsProtected; break;
    case c_COM2: SER_TXD = LPC178X_USART::c_SER2_TXD; SER_RXD = LPC178X_USART::c_SER2_RXD; PinsProtected = &COM2_PinsProtected; break;
    case c_COM3: SER_TXD = LPC178X_USART::c_SER3_TXD; SER_RXD = LPC178X_USART::c_SER3_RXD; PinsProtected = &COM3_PinsProtected; break;
    case c_COM4: SER_TXD = LPC178X_USART::c_SER4_TXD; SER_RXD = LPC178X_USART::c_SER4_RXD; PinsProtected = &COM4_PinsProtected; break;
    default: return;
    }
...
void LPC178X_USART_Driver::GetPins(int comPort, GPIO_PIN& rxPin, GPIO_PIN& txPin, GPIO_PIN& ctsPin, GPIO_PIN& rtsPin)
{
    switch(comPort)
    {
        case c_COM1: 
...
void  LPC178X_USART_Driver::BaudrateBoundary(int ComPortNum, UINT32& maxBaudrateHz, UINT32& minBaudrateHz)
{
    // same for both UART
...
BOOL LPC178X_USART_Driver::IsBaudrateSupported(int ComPortNum, UINT32 & BaudrateHz)
{
