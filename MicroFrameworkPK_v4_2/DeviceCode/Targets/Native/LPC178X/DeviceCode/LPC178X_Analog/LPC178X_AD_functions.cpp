...
#define LPC178X_AD_CHANNELS 6       // number of channels
#define LPC178X_AD_FIRST_PIN 32     // channel 0 pin (C0)
#define LPC178X_AD_FIRST_CHANNEL 10 // channel 0 hw channel
#define LPC178X_AD_SAMPLE_TIME 3    // sample time = 28.5 cycles
...
BOOL AD_Initialize( ANALOG_CHANNEL channel, INT32 precisionInBits )
{
    ...
    if (!(RCC->APB2ENR & RCC_APB2ENR_ADC1EN)) { // not yet initialized
        ...
        ADC1->SMPR1 = 0x00249249 * LPC178X_AD_SAMPLE_TIME;
        ADC1->SMPR2 = 0x09249249 * LPC178X_AD_SAMPLE_TIME;
    }
    // set pin as analog input
    CPU_GPIO_DisablePin(LPC178X_AD_FIRST_PIN + channel, RESISTOR_DISABLED, 0, GPIO_ALT_MODE_1);
#endif
    return TRUE;
}
...
INT32 AD_Read( ANALOG_CHANNEL channel )
{
    ...
    ADC1->SQR3 = LPC178X_AD_FIRST_CHANNEL + channel; // select channel
...
{
    return LPC178X_AD_CHANNELS;
}

GPIO_PIN AD_GetPinForChannel( ANALOG_CHANNEL channel )
{
    if ((UINT32)channel >= LPC178X_AD_CHANNELS) return GPIO_PIN_NONE;
    return LPC178X_AD_FIRST_PIN + channel;
}

BOOL AD_GetAvailablePrecisionsForChannel( ANALOG_CHANNEL channel, INT32* precisions, UINT32& size )
{
    size = 0;
    if (precisions == NULL || (UINT32)channel >= LPC178X_AD_CHANNELS) return FALSE;
    precisions[0] = 12;
    size = 1;
    return TRUE;
}
