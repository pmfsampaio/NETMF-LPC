...
#pragma arm section zidata = "g_LPC178X_EMAC_LWIP_Driver"
#endif

static LPC178X_EMAC_LWIP_Driver g_LPC178X_EMAC_LWIP_Driver;
static struct netif             g_LPC178X_EMAC_NetIF;
...
void lpc178x_status_callback(struct netif *netif)
{
...
err_t   lpc178x_ethhw_init( netif * myNetIf)
{ 
    ...
    myNetIf->linkoutput = LPC178X_EMAC_lwip_xmit;
    myNetIf->status_callback = lpc178x_status_callback;

    LPC178X_EMAC_lwip_open( myNetIf );
...
{
    InterruptTaskContinuation.InitializeCallback( (HAL_CALLBACK_FPN)LPC178X_EMAC_lwip_recv, pNetIf );
...
{
    return LPC178X_EMAC_LWIP_Driver::Bind();
}
int  Network_Interface_Open(int index)
{
    return LPC178X_EMAC_LWIP_Driver::Open(index);
}
BOOL Network_Interface_Close(int index)
{
    return LPC178X_EMAC_LWIP_Driver::Close();
}

int LPC178X_EMAC_LWIP_Driver::Open(int index)
{    
    ...
    len = g_LPC178X_EMAC_NetIF.hwaddr_len;
    if(len == 0 || iface->macAddressLen < len)
    {
        len = iface->macAddressLen;
        g_LPC178X_EMAC_NetIF.hwaddr_len = len;
    }
    memcpy(g_LPC178X_EMAC_NetIF.hwaddr, iface->macAddressBuffer, len);

    ipaddr.addr = iface->ipaddr;
    gateway.addr = iface->gateway;
    subnetmask.addr = iface->subnetmask;

    pNetIF = netif_add( &g_LPC178X_EMAC_NetIF, &ipaddr, &subnetmask, &gateway, NULL, lpc178x_ethhw_init, ethernet_input );
       
    /* Enable the INTERRUPT */                            
    CPU_INTC_ActivateInterrupt(NVIC_TO_INTC(ENET_IRQn), (HAL_CALLBACK_FPN)LPC178X_EMAC_lwip_interrupt, &g_LPC178X_EMAC_NetIF);
    ...
    return g_LPC178X_EMAC_NetIF.num;
}

BOOL LPC178X_EMAC_LWIP_Driver::Close( )
{
    LwipUpTimeCompletion.Abort();

    /* Disable the interrupt */
    CPU_INTC_DeactivateInterrupt(NVIC_TO_INTC(ENET_IRQn));

    InterruptTaskContinuation.Abort();

    LwipNetworkStatus = 0;

    netif_set_down( &g_LPC178X_EMAC_NetIF );
    netif_remove( &g_LPC178X_EMAC_NetIF );

    LPC178X_EMAC_lwip_close( &g_LPC178X_EMAC_NetIF );
    
    memset( &g_LPC178X_EMAC_NetIF, 0, sizeof g_LPC178X_EMAC_NetIF);

    return TRUE;
}

BOOL  LPC178X_EMAC_LWIP_Driver::Bind  ( )
{
    return TRUE;
}
