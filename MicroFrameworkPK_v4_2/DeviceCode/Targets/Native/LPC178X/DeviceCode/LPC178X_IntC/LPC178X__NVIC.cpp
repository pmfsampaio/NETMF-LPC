...
LPC178X_NVIC_Driver g_LPC178X_NVIC_Driver;

void CPU_INTC_Initialize()
{
    LPC178X_NVIC_Driver::Initialize();
}

BOOL CPU_INTC_ActivateInterrupt(UINT32 Irq_Index, HAL_CALLBACK_FPN ISR, void* ISR_Param)
{
    return LPC178X_NVIC_Driver::ActivateInterrupt(Irq_Index, ISR, ISR_Param);
}

BOOL CPU_INTC_DeactivateInterrupt(UINT32 Irq_Index)
{
    return LPC178X_NVIC_Driver::DeactivateInterrupt(Irq_Index);
}

BOOL CPU_INTC_InterruptEnable(UINT32 Irq_Index)
{
    return LPC178X_NVIC_Driver::InterruptEnable(Irq_Index);
}

BOOL CPU_INTC_InterruptDisable(UINT32 Irq_Index)
{
    return LPC178X_NVIC_Driver::InterruptDisable(Irq_Index);
}

BOOL CPU_INTC_InterruptEnableState(UINT32 Irq_Index)
{
    return LPC178X_NVIC_Driver::InterruptEnableState(Irq_Index);
}

BOOL CPU_INTC_InterruptState(UINT32 Irq_Index)
{
    return LPC178X_NVIC_Driver::InterruptState(Irq_Index);
}
...
void GlobalNVICISR()
{
    ...
    if (g_LPC178X_NVIC_Driver.intVector[vector_index])
    {
        g_LPC178X_NVIC_Driver.intVector[vector_index](g_LPC178X_NVIC_Driver.intParam[vector_index]);
    }
}

void LPC178X_NVIC_Driver::Initialize()
{
...
BOOL LPC178X_NVIC_Driver::ActivateInterrupt(UINT32 Irq_Index, HAL_CALLBACK_FPN ISR, void* ISR_Param)
{
    GLOBAL_LOCK( irq);

    // point user vector to the request one
    g_LPC178X_NVIC_Driver.intVector[Irq_Index] = ISR;
    g_LPC178X_NVIC_Driver.intParam[Irq_Index] = ISR_Param;
...
BOOL LPC178X_NVIC_Driver::DeactivateInterrupt(UINT32 Irq_Index)
{
    GLOBAL_LOCK( irq);
    ...
    // clear user vector
    g_LPC178X_NVIC_Driver.intVector[Irq_Index] = 0;
    g_LPC178X_NVIC_Driver.intParam[Irq_Index] = 0;

    return TRUE;
}

BOOL LPC178X_NVIC_Driver::InterruptEnable(UINT32 Irq_Index)
{
    GLOBAL_LOCK( irq);
    ...
    else if (INTC_TO_NVIC(Irq_Index) == USB_IRQn)
    {
        // set interrupt priority
...
BOOL LPC178X_NVIC_Driver::InterruptDisable(UINT32 Irq_Index)
{
    GLOBAL_LOCK( irq);
...
BOOL LPC178X_NVIC_Driver::InterruptEnableState(UINT32 Irq_Index)
{
    if (INTC_TO_NVIC(Irq_Index) == SysTick_IRQn)
    {
...
BOOL LPC178X_NVIC_Driver::InterruptState(UINT32 Irq_Index)
{
