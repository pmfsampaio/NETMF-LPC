...
{
    return LPC178X_LCDC_Driver::Initialize( config );
}

BOOL LCD_Controller_Uninitialize()
{
    return LPC178X_LCDC_Driver::Uninitialize();
}

BOOL LCD_Controller_Enable(BOOL fEnable)
{
    return LPC178X_LCDC_Driver::Enable( fEnable );
}

UINT32* LCD_GetFrameBuffer()
{
    return LPC178X_LCDC_Driver::GetScreenBuffer();
}
