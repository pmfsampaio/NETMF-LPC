...
#include "..\CMSIS\include\lpc177x_8x_pinsel.h"

//--//

#if defined(ADS_LINKER_BUG__NOT_ALL_UNUSED_VARIABLES_ARE_REMOVED)
#pragma arm section rodata = "c_Gpio_Attributes_LPC178X"
#endif

const UINT8 __section(rodata) LPC178X_GPIO_Driver::c_Gpio_Attributes[LPC178X_GPIO_Driver::c_MaxPins] =
{
...
#if defined(ADS_LINKER_BUG__NOT_ALL_UNUSED_VARIABLES_ARE_REMOVED)
#pragma arm section zidata = "g_LPC178X_GPIO_Driver"
#endif

LPC178X_GPIO_Driver g_LPC178X_GPIO_Driver;
LPC178X_GPIO_Driver g_LPC178X_PINSEL_Driver;
...
{
    return LPC178X_GPIO_Driver::Initialize();
}

BOOL CPU_GPIO_Uninitialize()
{
    return LPC178X_GPIO_Driver::Uninitialize();
}

UINT32 CPU_GPIO_Attributes( GPIO_PIN Pin )
{
    return LPC178X_GPIO_Driver::Attributes( Pin );
}

void CPU_GPIO_DisablePin( GPIO_PIN Pin, GPIO_RESISTOR ResistorState, UINT32 Direction, GPIO_ALT_MODE AltFunction )
{
    LPC178X_GPIO_Driver::DisablePin( Pin, ResistorState, Direction, AltFunction);
}

void CPU_GPIO_EnableOutputPin( GPIO_PIN Pin, BOOL InitialState )
{
    LPC178X_GPIO_Driver::EnableOutputPin( Pin, InitialState );
}
...
{
    return LPC178X_GPIO_Driver::EnableInputPin( Pin, GlitchFilterEnable, ISR, 0, IntEdge, ResistorState );
}
...
{
    return LPC178X_GPIO_Driver::EnableInputPin( Pin, GlitchFilterEnable, ISR, ISR_Param, IntEdge, ResistorState );
}

BOOL CPU_GPIO_GetPinState( GPIO_PIN Pin )
{
    return LPC178X_GPIO_Driver::GetPinState( Pin );
}

void CPU_GPIO_SetPinState( GPIO_PIN Pin, BOOL PinState )
{
    LPC178X_GPIO_Driver::SetPinState( Pin, PinState );
}

INT32 CPU_GPIO_GetPinCount()
{
    return LPC178X_GPIO_Driver::GetPinCount();
}

void CPU_GPIO_GetPinsMap( UINT8* pins, size_t size )
{
    LPC178X_GPIO_Driver::GetPinsMap( pins, size );
}

UINT8 CPU_GPIO_GetSupportedResistorModes(GPIO_PIN pin)
{
    return LPC178X_GPIO_Driver::GetSupportedResistorModes( pin);
}

UINT8 CPU_GPIO_GetSupportedInterruptModes(GPIO_PIN pin)
{
    return LPC178X_GPIO_Driver::GetSupportedInterruptModes( pin );
}

BOOL CPU_GPIO_PinIsBusy( GPIO_PIN Pin )
{
    return LPC178X_GPIO_Driver::PinIsBusy( Pin );
}

BOOL CPU_GPIO_ReservePin( GPIO_PIN Pin, BOOL fReserve )
{
    return LPC178X_GPIO_Driver::ReservePin( Pin, fReserve );
}

UINT32 CPU_GPIO_GetDebounce()
{
    return LPC178X_GPIO_Driver::GetDebounce();
}

BOOL CPU_GPIO_SetDebounce( INT64 debounceTimeMilliseconds )
{
    return LPC178X_GPIO_Driver::SetDebounce( debounceTimeMilliseconds );
}
...
//  Initialize
///////////////////////////////////////////////////////////////////////////////
BOOL LPC178X_GPIO_Driver::Initialize()
{
    int i;
    UINT32 port;
    LPC178X_GPIO& GPIO = LPC178X::GPIO();

    /* Enable fast GPIO on Port 0 & 1 */
//    LPC178X::SYSCON().SCS |= LPC178X_SYSCON::HS_IO;

    for (port=0; port < c_MaxPins; port++)
    {
        g_LPC178X_GPIO_Driver.m_PinReservationInfo[port] = FALSE;
    }

    // initialize the interrupt information
    {
        PIN_ISR_DESCRIPTOR* pinIsr = g_LPC178X_GPIO_Driver.m_PinIsr;

        for(i = 0; i < c_MaxPins; i++)
        {
            pinIsr->m_pin     = i;
            pinIsr->m_intEdge = GPIO_INT_NONE;
            pinIsr->m_isr     = LPC178X_GPIO_Driver::STUB_ISRVector;
            pinIsr->m_param   = NULL;
    ...
    if(!CPU_INTC_ActivateInterrupt( NVIC_TO_INTC(LPC178X_GPIO::c_GPIO_Irq), GPIO_ISR, NULL )) return FALSE;
    return TRUE;
}
...
BOOL LPC178X_GPIO_Driver::Uninitialize()
{
    GLOBAL_LOCK(irq);
    ...
    {
        PIN_ISR_DESCRIPTOR* pinIsr = g_LPC178X_GPIO_Driver.m_PinIsr;
        LPC178X_GPIOIRQ& GPIOIRQ = LPC178X::GPIOIRQ();
...
#if 0
    if(!CPU_INTC_DeactivateInterrupt( LPC178X_VIC::c_IRQ_INDEX_EINT3 )) return FALSE;
#endif
    return TRUE;
}
...
UINT32 LPC178X_GPIO_Driver::Attributes( GPIO_PIN pin )
{
...
void LPC178X_GPIO_Driver::DisablePin( GPIO_PIN pin, GPIO_RESISTOR resistorState, UINT32 Direction, GPIO_ALT_MODE AltFunction )
{
    UINT32 port, bit, temp;
    ...
    PINSEL_ConfigPin((UINT8)port, (UINT8)bit, (UINT8)AltFunction);

    // Disable ISR
    PIN_ISR_DESCRIPTOR& PinIsr = g_LPC178X_GPIO_Driver.m_PinIsr[pin];
...
void LPC178X_GPIO_Driver::EnableOutputPin( GPIO_PIN pin, BOOL initialState )
{
    ASSERT(pin < c_MaxPins);
...
//  EnableInputPin
///////////////////////////////////////////////////////////////////////////////
BOOL LPC178X_GPIO_Driver::EnableInputPin( GPIO_PIN pin, BOOL GlitchFilterEnable, GPIO_INTERRUPT_SERVICE_ROUTINE ISR, void* Param, GPIO_INT_EDGE intEdge, GPIO_RESISTOR resistorState )
{
    ASSERT(pin < c_MaxPins);
    ...
    if((Port != 0) && (Port != 2))  //Only port 0 & 2 can generate interrupts on the LPC178X
    {
        return FALSE;
    }
    switch (resistorState)
    {
        ...
        case RESISTOR_PULLUP:
        	PINSEL_SetPinMode(Port, Bit, PINSEL_BASICMODE_PULLUP);
...
BOOL LPC178X_GPIO_Driver::GetPinState( GPIO_PIN pin )
{
    ASSERT(pin < c_MaxPins);
...
//  SetPinState
///////////////////////////////////////////////////////////////////////////////
void LPC178X_GPIO_Driver::SetPinState( GPIO_PIN pin, BOOL pinState )
{
...
//  PinIsBusy
///////////////////////////////////////////////////////////////////////////////
BOOL LPC178X_GPIO_Driver::PinIsBusy( GPIO_PIN pin )
{
    return g_LPC178X_GPIO_Driver.m_PinReservationInfo[pin];
}

///////////////////////////////////////////////////////////////////////////////
//  ReservePin
///////////////////////////////////////////////////////////////////////////////
BOOL LPC178X_GPIO_Driver::ReservePin( GPIO_PIN pin, BOOL fReserve )
{
    GLOBAL_LOCK(irq);

    g_LPC178X_GPIO_Driver.m_PinReservationInfo[pin] = fReserve;
...
//  GetDebounce
///////////////////////////////////////////////////////////////////////////////
UINT32 LPC178X_GPIO_Driver::GetDebounce()
{

    return CPU_TicksToTime( g_LPC178X_GPIO_Driver.m_DebounceTicks ) / 10000;
}

///////////////////////////////////////////////////////////////////////////////
//  SetDebounce
///////////////////////////////////////////////////////////////////////////////
BOOL LPC178X_GPIO_Driver::SetDebounce( INT64 debounceTimeMilliseconds )
{   
    ...
    g_LPC178X_GPIO_Driver.m_DebounceTicks = CPU_MillisecondsToTicks( (UINT32)debounceTimeMilliseconds );

    return TRUE;
}

INT32 LPC178X_GPIO_Driver::GetPinCount()
{
    return c_MaxPins;
}

void LPC178X_GPIO_Driver::GetPinsMap( UINT8* pins, size_t size )
{
...
UINT8 LPC178X_GPIO_Driver::GetSupportedResistorModes( GPIO_PIN pin)
{
    return LPC178X_GPIO_Driver::c_GPIO_ResistorMode;
}


UINT8 LPC178X_GPIO_Driver::GetSupportedInterruptModes( GPIO_PIN pin)
{

    return LPC178X_GPIO_Driver::c_GPIO_InterruptMode;
}

void LPC178X_GPIO_Driver::STUB_ISRVector( GPIO_PIN Pin, BOOL PinState, void* Param )
{
    lcd_printf( "\fSTUB_GPI %04x\r\n", (size_t)Param );
    //HARD_BREAKPOINT();
}

void LPC178X_GPIO_Driver::GPIO_ISR( void* Param )
{
    ...
    for(bit = 0; (GPIOIRQ.IOIntStatus & 0x1) && (bit < 32); bit++) //exit when there are no pending IRQs or we reach P0.31 (should always be the first)
    {
        ...
        pinIsr = &g_LPC178X_GPIO_Driver.m_PinIsr[ bit /*+ 0 * LPC178X_GPIO_Driver::c_PinsPerPort*/ ];
        
        //Debounce
        if( (pinIsr->m_flags & PIN_ISR_DESCRIPTOR::c_Flags_Debounce)&&
            (Time_CurrentTicks() - pinIsr->m_lastExecTicks < g_LPC178X_GPIO_Driver.m_DebounceTicks))
    ...
    for(bit = 0; (GPIOIRQ.IOIntStatus & 0x4) && (bit < 32); bit++) // as above, but the guard value is P2.31
    {
        ...
        pinIsr = &g_LPC178X_GPIO_Driver.m_PinIsr[ bit + 2 * LPC178X_GPIO_Driver::c_PinsPerPort ];
...
void LPC178X_GPIO_Driver::PIN_ISR_DESCRIPTOR::Fire( void* arg )
{
